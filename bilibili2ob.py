...
class Bilibili2Obsidian:
    ...
    async def process(self, url):
        ...
        if not info["video_url"]:
            print("Error: Could not find video URL for Bilibili. Bilibili uses dynamic streaming, try installing yt-dlp for better results.")
            return
        ...
        # 4. Transcribe
        temp_audio = "temp_bili_audio.wav"
        ...
        print(f"\nDone! Bilibili note saved to: {output_path}")
    ...
    def generate_md(self, info, segments, video_filename):
        ...
        body.append("## 资料来源")
        body.append(f"- [B站原视频]({info['page_url']}) - 访问日期：{datetime.datetime.now().strftime('%Y-%m-%d')}")
    ...
    def get_safe_filename(self, info):
        ...
        return f"{date_str}_Bili_{safe_title}.md"

async def main():
    if len(sys.argv) < 2:
        print("Usage: python bilibili2ob.py <url>")
        return
    url = sys.argv[1]
