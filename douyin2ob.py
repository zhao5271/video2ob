...
class Douyin2Obsidian:
    ...
    async def process(self, url):
        ...
        if not info["video_url"]:
            print("Error: Could not find video URL. Please try again or check the link.")
            return
    ...
    def generate_md(self, info, segments, video_filename):
        ...
        body.append("## 资料来源")
        body.append(f"- [抖音原视频]({info['page_url']}) - 访问日期：{datetime.datetime.now().strftime('%Y-%m-%d')}")
    ...
    def get_safe_filename(self, info):
        ...
        return f"{date_str}_DY_{safe_title}.md"

async def main():
    if len(sys.argv) < 2:
        print("Usage: python douyin2ob.py <douyin_url>")
        return
